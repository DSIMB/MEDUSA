...
def ali_long_pir(filename):
	'''Alignment lenght in line number'''
	f = open(filename)
	l = f.readline()
...
def pir2list(filename):
	'''Parssing of pir alignment in list of list'''
...
def fasta2list(filename):
	'''Parssing of fasta alignment in list of list'''
...
def Rc1(mat_counts):
	"""  Number of diffrent résidus in each column""" 
	R = []
...
def get_args():
	...
	(default 99 %%) -gtc gap_treshold_column (default 100 %%)"
	...
	parser.add_argument('-gts', dest = "gap_treshold_seq", default = 99,
	type = float, help = "Sequences with a percentage of gaps superior than the\
	gap_treshold were not considered" )
	parser.add_argument('-gtc', dest = "gap_treshold_col", default = 100, 
