...
my $soft="/home/defense/gelly/serenity/scratch/gelly/SOFT";


#my $bioprogs_dir="/home/soeding/programs";   # see next two lines
my $ncbidir="$soft/PSI_BLAST/blast-2.2.18";           # Put the directory path with the BLAST executables 
#my $perl="/home/wagram/gelly/CORNICHON/BIN/HHSUITE/HHSEARCH";                 # Put the directory path where reformat.pl is lying
#my $dummydb="$soft/hhsuite-2.0.16/data"; # Put the name given to the dummy blast directory (or leave this name)
my $dummydb="$soft/hh-suite/data";


my $psipreddir="$soft/PSIPRED";     # Put the directory path with the PSIPRED executables 
...
Usage:   perl $0 <ali file> [<outfile>] [-fas|-a3m|-clu|sto]  
  or     perl $0 <HMM file>  <outfile>   -hmm  

Example: perl $0 1T2KD_1_1.a3m 1T2KD_1_1.fas
\n";
...
if ($informat ne "hmm")
{
    ...
    #&System("perl $soft/hh-suite/scripts/reformat.pl -v $v2 -M first $informat a3m $infile $inbase.in.a3m");
    &System("perl $soft/hh-suite/scripts/reformat.pl -v $v2 -M first $informat a3m $infile $inbase.a3m");
       
    # Read query sequence
#    open (INFILE, "<$inbase.in.a3m") or die ("ERROR: cannot open $inbase.in.a3m: $!\n");
    open (INFILE, "<$inbase.a3m") or die ("ERROR: cannot open $inbase.a3m: $!\n");
    ...
    &System("$soft/hh-suite/scripts/reformat.pl -v $v2 -r -noss a3m psi $inbase.a3m $inbase.psi");
...
## OLD PSIPRED		&System("$execdir/psipred $inbase.mtx $datadir/weights.dat $datadir/weights.dat2 $datadir/weights.dat3 $datadir/weights.dat4 > $inbase.ss"); 
#
#	# READ PSIPRED file
...
#	}
    ...
    close(OUTFILE);
    close(INFILE);
    unlink("$inbase.pn $inbase.sn $inbase.mn $inbase.chk $inbase.blalog $inbase.aux $inbase.ss $inbase.ss2");
...
# Does dummy database exist?
	if (!-e "$dummydb.phr") {
		...
		&System("$ncbidir/bin/formatdb -i $dummydb");
	}

# Start Psiblast from checkpoint file tmp.chk that was generated to build the profile
	&System("$ncbidir/bin/blastpgp -t 1 -b 1 -j 1 -h 0.001 -d $dummydb -i $infile -B $inbase.psi -Q $inbase.mat -C $inbase.chk 1> $inbase.blalog 2> $inbase.blalog");

#    print("Predicting secondary structure...\n");

	system("echo $inroot.chk > $inbase.pn\n");
	system("echo $inroot.sq > $inbase.sn\n");
	system("$ncbidir/bin/makemat -P $inbase");
