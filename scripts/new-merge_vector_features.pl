...
#OPEN AAINDEX
open(F1,"$name.vector_aaindex") or die "Cannot open file \"$name.vector_aaindex\" : $!\n";	
my @tab_file1=<F1>;
close F1;


####### WITH GAP #######
if (not -e "$name.vector_aamtx_gaps")
{
	print STDERR "File \"$name.vector_aamtx_gaps\" not exist!\n";
	exit 1;
#next FOREACH13;
}
...
####### ONEHOT SEQ #######
if (not -e "$name.vector_seqonehot")
{
	print STDERR "File \"$name.vector_seqonehot\" not exist!\n";
	exit 1;
#next FOREACH13;
}


open(F3,"$name.vector_seqonehot") or die "Cannot open file \"$name.vector_seqonehot\" : $!\n";	
my @tab_file3=<F3>;
close F3;
...
{
	print STDERR "Not the same file size : \"$i\" and \"$name.vector_aamtx_gaps\" ( $#tab_file1 != $#tab_file2 ) !\n";
	exit 1;
#next FOREACH13;
}
...
{
	print STDERR "Not the same file size : \"$i\" and \"$name.vector_seqonehot\" ( $#tab_file1 != $#tab_file3 ) !\n";
	exit 1;
#next FOREACH13;
}
...
#OPEN AAMTX
#CHECK
if (not -e "$name.vector_aamtx")
{
	print STDERR "File \"$name.vector_aamtx\" not exist!\n";
	exit 1;
}
...
if ($#tab_file1 != $#tab_file2)
{
	print STDERR "Not the same file size : \"$i\" and \"$name.vector_aamtx\" ( $#tab_file1 != $#tab_file2 )!\n";
	exit 1;
}
