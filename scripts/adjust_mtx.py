...
def get_limit_query_seq(hits):
	begins = []
	for i in hits:
		if i[:6]=="Query:":
			begins.append(int(i[6:10].replace(' ','')))
	
	ends = []
	for i in hits:
                if i[:6]=="Query:":
			ends.append(int(i[-4:-1].replace(' ','')))
	pos_begin = min(begins)
	pos_end = max(ends)
	return pos_begin-1, pos_end-1
...
def get_args():
	...
	parser.add_argument('-in3', dest = "seq_tokeep", type = str,
        help = "seq to keep")
	parser.add_argument('-n', dest = "nround", type = int, default = 3,
        help = "number of psiblast round")
	
	

        args = parser.parse_args()

        return (args.blast_out, args.mtx_file, args.nround, args.seq_tokeep)


def main():
	blast_out, mtx_file ,nround, seqtokeep = get_args()
	pos_begin , pos_end = get_limit_query_seq(get_last_round(blast_out,nround))
